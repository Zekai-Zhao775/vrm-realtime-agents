...
A Next.js application that combines OpenAI's Realtime API with 3D VRM avatars for interactive voice conversations. This project integrates the advanced voice agent patterns from OpenAI Realtime API Agents Demo with ChatVRM's 3D avatar system.

## Features

- **Real-time voice conversations** using OpenAI's Realtime API and OpenAI Agents SDK
- **3D VRM avatar rendering** with Three.js and @pixiv/three-vrm
- **Multiple agent types**: Chat Supervisor, Simple Handoff, Customer Service, Virtual Therapist
- **WebRTC audio streaming** for low-latency voice interaction
- **VRM avatar integration** with lip sync and facial expressions

## Project Structure

This project combines two main components:
- **openai-realtime-agents**: Advanced AI voice agent system with multi-agent patterns
- **ChatVRM**: 3D VRM avatar rendering and animation system

## Pages

- `/` - Original OpenAI Realtime Agents interface with debugging tools
- `/vrm-chat/` - VRM avatar chat interface with integrated voice agents

---

# Original OpenAI Realtime API Agents Demo
